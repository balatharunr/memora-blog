...
export const updateUserProfile = async (userId, profileData) => {
  ...
  try {
    ...
    // Update authorName in all posts by this user if name was changed
    if (profileData.name) {
      ...
      // Commit the batch
      await batch.commit();
      console.log(`Updated username in ${postsSnapshot.size} posts`);
      
      // Also update the user's name in comments
      try {
        ...
        if (!commentsSnapshot.empty) {
          ...
          console.log(`Updated username in ${commentsSnapshot.size} comments`);
        }
      } catch (commentError) {
        console.error('Error updating comments with new username:', commentError);
        // Continue execution even if comment updates fail
      }
      
      // Update notifications where the user is the sender
      try {
        ...
        if (!notificationsSnapshot.empty) {
          ...
          notificationsSnapshot.forEach((notifDoc) => {
            notificationBatch.update(notifDoc.ref, {
              fromUserName: profileData.name
            });
          });
          
          await notificationBatch.commit();
          console.log(`Updated username in ${notificationsSnapshot.size} notifications`);
        }
      } catch (notifError) {
        console.error('Error updating notifications with new username:', notifError);
        // Continue execution even if notification updates fail
      }
    }
    
    // Set a flag in localStorage to trigger session updates across tabs
...
export const updatePost = async (postId, postData) => {
  try {
    ...
    // Return the updated post with all the original data
    return { 
      id: postId, 
      ...existingPostData, 
      ...updatedPost,
      // Convert server timestamp to JS date for immediate use in UI
      updatedAt: new Date(),
    };
  } catch (error) {
...
export const deleteComment = async (commentId) => {
  try {
    // Get comment data first to update the post's comment count
